...
enum Cache {
    ...
    static func get<T: Codable>(key: String, as: T.Type, folder: String? = nil) throws -> T {
    ...
    static func set<T: Codable>(key: String, object: T, folder: String? = nil) throws {
        let file = try self.path(for: key, folder: folder)
    ...
    ///   - folder: The optional subfolder to store the item
    /// - Throws: an error if it can't be saved
    static func delete(key: String, folder: String? = nil) throws {
        let file = try self.path(for: key, folder: folder)
